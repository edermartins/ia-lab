...
def train_logistic_regression(train_data: pd.DataFrame, test_data: pd.DataFrame) -> dict:
    ...
    print("Colunas disponíveis nas previsões:", predictions.columns.tolist())
    print("Formato das probabilidades:", probabilities.shape)
    ...
    return {"model": lr_model, "log_loss": log_loss_value}
...
def train_decision_tree(train_data: pd.DataFrame, test_data: pd.DataFrame) -> dict:
    ...
    # Calcula métricas
    log_loss_value = log_loss(test_data['shot_made_flag'], probabilities[:, 1])
    f1_value = f1_score(test_data['shot_made_flag'], predictions['shot_made_flag'])
